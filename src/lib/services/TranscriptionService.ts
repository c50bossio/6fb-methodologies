...
export class TranscriptionService implements ITranscriptionService {
  ...
  async transcribeAudio(
    ...
    try {
      ...
      if (result.success) {
        ...
        this.log(`Transcription completed with ${result.data.provider}`, 'info')
      }
  ...
  async transcribeRecording(
    ...
    try {
      ...
      return {
        success: false,
        error: 'Recording retrieval not implemented - requires AudioRecordingService integration',
        code: 'INTEGRATION_REQUIRED'
      }
    } catch (error) {
      const message = error instanceof Error ? error.message : 'Recording transcription failed'
      this.log(`Recording transcription error: ${message}`, 'error')
      return { success: false, error: message, code: 'RECORDING_TRANSCRIPTION_FAILED' }
    }
  }

  async getTranscription(id: string): Promise<ServiceResult<TranscriptionResult>> {
  ...
  async getProviderStatus(): Promise<ServiceResult<Record<TranscriptionProvider, boolean>>> {
  ...
  private validateAudioBlob(audioBlob: Blob): ServiceResult<void> {
    if (!audioBlob || audioBlob.size === 0) {
      return { success: false, error: 'Invalid audio blob', code: 'INVALID_AUDIO' }
    }

    // Check file size (25MB limit for most providers)
    const maxSize = 25 * 1024 * 1024
    if (audioBlob.size > maxSize) {
      return { success: false, error: 'Audio file too large', code: 'FILE_TOO_LARGE' }
    }

    // Check MIME type
    const supportedTypes = ['audio/webm', 'audio/wav', 'audio/mp3', 'audio/mp4', 'audio/mpeg']
    if (!supportedTypes.some(type => audioBlob.type.includes(type))) {
      return { success: false, error: 'Unsupported audio format', code: 'UNSUPPORTED_FORMAT' }
    }
  ...
  private async transcribeWithProvider(
    ...
    try {
      switch (provider) {
        ...
        default:
          return { success: false, error: `Unknown provider: ${provider}`, code: 'UNKNOWN_PROVIDER' }
      }
    } catch (error) {
      const message = error instanceof Error ? error.message : 'Provider transcription failed'
  ...
  private async transcribeWithOpenAI(
    ...
    if (!providerConfig?.apiKey) {
      return { success: false, error: 'OpenAI API key not configured', code: 'CONFIG_MISSING' }
    }
  ...
  private formatOpenAIResponse(data: any, config: TranscriptionConfig): TranscriptionResult {
    ...
    if (data.segments) {
      data.segments.forEach((segment: any, index: number) => {
        segments.push({
          ...
          speaker: config.enableSpeakerIdentification ? `Speaker ${index % 2 + 1}` : undefined
        })
      })
    } else {
    ...
    return {
      id: this.generateId(),
      text: data.text,
      confidence: segments.length > 0 ? segments.reduce((sum, s) => sum + s.confidence, 0) / segments.length : 0.8,
      segments,
  ...
  private async checkProviderHealth(): Promise<void> {
    ...
    const healthChecks = Object.keys(this.providerConfigs).map(async (provider) => {
      try {
