...
export class NotesService implements INotesService {
  ...
  async createNote(
    ...
    try {
      ...
      this.log(`Note created: ${newNote.id} for user ${newNote.userId}`, 'info')
  ...
  async updateNote(
    ...
    try {
      ...
      if (updates.id && updates.id !== id) {
        return { success: false, error: 'Cannot change note ID', code: 'INVALID_UPDATE' }
      }
  ...
  async searchNotes(filter: NotesSearchFilter): Promise<ServiceResult<SessionNote[]>> {
    try {
      ...
      if (filter.userId) {
        ...
        candidateNotes = userNoteIds.map(id => this.notes.get(id)!).filter(Boolean)
      } else if (filter.sessionId) {
        const sessionNoteIds = this.notesBySession.get(filter.sessionId) || []
        candidateNotes = sessionNoteIds.map(id => this.notes.get(id)!).filter(Boolean)
      } else {
      ...
      const filteredNotes = candidateNotes.filter(note => {
        ...
        if (filter.sessionId && !filter.userId && note.sessionId !== filter.sessionId) {
          return false
        }
  ...
  async linkTranscription(noteId: string, transcriptionId: string): Promise<ServiceResult<void>> {
    try {
      ...
      this.log(`Transcription linked to note: ${noteId} -> ${transcriptionId}`, 'info')
      return { success: true, data: undefined }
  ...
  async linkAudioRecording(noteId: string, recordingId: string): Promise<ServiceResult<void>> {
    try {
      const note = this.notes.get(noteId)
      if (!note) {
        return { success: false, error: 'Note not found', code: 'NOT_FOUND' }
      ...
      this.notes.set(noteId, updatedNote)
      this.log(`Audio recording linked to note: ${noteId} -> ${recordingId}`, 'info')
      return { success: true, data: undefined }

    } catch (error) {
      const message = error instanceof Error ? error.message : 'Failed to link audio recording'
  ...
  async getNoteStats(userId?: string, sessionId?: string): Promise<ServiceResult<{
    total: number
  ...
  private validateNoteData(note: Partial<SessionNote>): ServiceResult<void> {
    ...
    if (!note.sessionId?.trim()) {
      return { success: false, error: 'Session ID is required', code: 'VALIDATION_FAILED' }
    }
    ...
    if (!note.content?.trim()) {
      return { success: false, error: 'Content is required', code: 'VALIDATION_FAILED' }
    }
    ...
    if (note.content && note.content.length > 10000) {
      return { success: false, error: 'Content too long (max 10000 characters)', code: 'VALIDATION_FAILED' }
    }

    const validCategories: NoteCategory[] = ['session-note', 'insight', 'action-item', 'question', 'reflection']
    if (note.category && !validCategories.includes(note.category)) {
      return { success: false, error: 'Invalid note category', code: 'VALIDATION_FAILED' }
    }

    const validStatuses: NoteStatus[] = ['draft', 'published', 'archived']
    if (note.status && !validStatuses.includes(note.status)) {
      return { success: false, error: 'Invalid note status', code: 'VALIDATION_FAILED' }
    }
