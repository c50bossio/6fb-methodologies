  ...
  try {
    ...
    if (!fs.existsSync(CSV_FILE_PATH)) {
      ...
      return {
        success: false,
        count: 0,
        error: 'CSV file not found. Please ensure skool-members.csv exists in /data directory.'
      }
    }
    ...
    // Find column indices
    const firstNameIdx = headers.findIndex(h => h.includes('firstname') || h.includes('first'))
    const lastNameIdx = headers.findIndex(h => h.includes('lastname') || h.includes('last'))
    const emailIdx = headers.findIndex(h => h.includes('email'))
    const invitedByIdx = headers.findIndex(h => h.includes('invited'))
    const joinedDateIdx = headers.findIndex(h => h.includes('joined') || h.includes('date'))
    ...
    console.log(`✅ CSV loaded: ${processed} members processed, ${skipped} skipped`)
