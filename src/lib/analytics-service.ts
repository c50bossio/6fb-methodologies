...
interface ConversionEvent {
  type: 'purchase' | 'registration_start' | 'registration_complete' | 'payment_complete'
  value?: number
  currency?: string
...
class GoogleAnalyticsService {
  ...
  async trackServerEvent(event: AnalyticsEvent) {
    ...
    try {
      const response = await fetch(`${endpoint}?measurement_id=${this.measurementId}&api_secret=${this.apiSecret}`, {
        method: 'POST',
        headers: {
...
class FacebookPixelService {
  ...
  async trackServerEvent(event: string, userData: any, customData: any, eventId?: string) {
    if (!this.accessToken) {
      console.warn('Facebook Access Token not configured for server-side tracking')
      return
    }
...
class AnalyticsIntegrationService {
  ...
  constructor(config: {
    ...
    this.ga4 = new GoogleAnalyticsService(config.ga4MeasurementId, config.ga4ApiSecret, config.debug)
    this.fbPixel = new FacebookPixelService(config.fbPixelId, config.fbAccessToken, config.debug)
  ...
  async trackWorkshopRegistration(data: {
    ...
    await Promise.all([
      ...
      this.fbPixel.trackServerEvent('Purchase', {
        em: this.hashEmail(data.customerEmail),
        ph: data.customerPhone ? this.hashPhone(data.customerPhone) : undefined,
  ...
  trackWorkshopEvent(event: string, properties: Record<string, any> = {}) {
    ...
    this.fbPixel.trackEvent(`Workshop${event.charAt(0).toUpperCase() + event.slice(1)}`, properties)
  }
...
export function useAnalytics() {
  return {
    ...
    trackWorkshopEvent: analyticsService.trackWorkshopEvent.bind(analyticsService),
...
export { GoogleAnalyticsService, FacebookPixelService, AnalyticsIntegrationService }