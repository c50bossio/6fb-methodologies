...
export const LessonTypeSchema = z.enum(
  ['video', 'audio', 'text', 'interactive', 'exercise', 'quiz'],
  {
...
export const ModuleContentSchema = z.object({
  ...
  learningObjectives: z
    ...
    .min(1, 'At least one learning objective required'),
...
export const WorkshopModuleSchema = z.object({
  ...
  moduleOrder: z
    .number()
    .int()
    .min(1, 'Module order must be positive')
    .max(50, 'Module order too high'),
...
export const validateModuleAccess = (
  ...
  moduleRequiredTier?: SubscriptionTier
): boolean => {
...
export const createBodyValidationMiddleware = <T extends z.ZodType>(
  schema: T
) => {
...
export const createQueryValidationMiddleware = <T extends z.ZodType>(
  schema: T
) => {
  return (data: unknown): z.infer<T> => {
    const result = schema.safeParse(data);
...
export const createParamsValidationMiddleware = <T extends z.ZodType>(
  schema: T
) => {
  return (data: unknown): z.infer<T> => {
    const result = schema.safeParse(data);
...
export const validateModulesQuery = createQueryValidationMiddleware(
  GetModulesQuerySchema
);
export const validateLessonsQuery = createQueryValidationMiddleware(
  GetLessonsQuerySchema
);
export const validateUpdateProgressBody = createBodyValidationMiddleware(
  UpdateProgressRequestSchema
);
export const validateCompleteLessonBody = createBodyValidationMiddleware(
  CompleteLessonRequestSchema
);
export const validateCompleteModuleBody = createBodyValidationMiddleware(
  CompleteModuleRequestSchema
);

// Common parameter validators
export const validateUuidParam = createParamsValidationMiddleware(
  z.object({
    id: z.string().uuid('Invalid ID format'),
  })
);

export const validateModuleIdParam = createParamsValidationMiddleware(
  z.object({
    moduleId: z.string().uuid('Invalid module ID format'),
  })
);

export const validateLessonIdParam = createParamsValidationMiddleware(
  z.object({
...
export const validateModuleQueryResult = (data: unknown): WorkshopModule => {
  ...
  if (!result.success) {
    throw new Error(
      `Invalid module data from database: ${result.error.message}`
    );
  }
  return result.data;
};
...
export const validateProgressQueryResult = (data: unknown): UserProgress => {
  ...
  if (!result.success) {
    throw new Error(
      `Invalid progress data from database: ${result.error.message}`
    );
  }
  return result.data;
};
