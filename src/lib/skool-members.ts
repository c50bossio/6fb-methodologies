...
const verifiedSkoolMembers = globalThis.__verifiedSkoolMembers ?? new Map<string, SkoolMember>()
...
export function removeVerifiedSkoolMember(email: string): boolean {
  ...
  if (existed) {
    console.log('🗑️ Skool member removed from verification database:', normalizedEmail)
  }
...
export function getSkoolMemberStats() {
  ...
  return {
    ...
    recentMembers: members
      ...
      .slice(0, 5),
    oldestMember: members.length > 0 ?
      members.reduce((oldest, current) =>
        new Date(current.verifiedAt) < new Date(oldest.verifiedAt) ? current : oldest
      ) : null
  }
}