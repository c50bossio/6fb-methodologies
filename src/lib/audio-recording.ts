...
export function detectBrowserCompatibility(): BrowserCompatibility {
  ...
  const aacSupport = MediaRecorder.isTypeSupported(
...
export class AudioRecorder {
    ...
    try {
      ...
      const recording: AudioRecording = {
        ...
        metadata: {
          ...
          sampleRate: audioQualityConfigs[this.config.quality]
            .sampleRate as number,
          channels: audioQualityConfigs[this.config.quality]
            .channelCount as number,
          mimeType,
          sessionInfo,
        },
      };
  ...
  async saveRecording(
    ...
    try {
      ...
      const recordings =
        loadUserProgress<AudioRecording[]>(userId, 'audio_recordings') || [];
      recordings.push({
        ...recording,
  ...
  private async generateNoteFromRecording(
    userId: string,
    recording: AudioRecording
  ): Promise<string> {
  ...
  private setState(
    ...
    additionalData?: Partial<AudioRecorderState>
  ): void {
...
export function createSessionRecorder(
  ...
  recorder.on(
    ...
    ({ isActive, timestamp }: { isActive: boolean; timestamp: number }) => {
      if (sessionInfo.speaker && isActive) {
        // Could trigger auto-note capture based on speaker activity
        console.log(
          `Voice activity detected for ${sessionInfo.speaker} at ${timestamp}`
        );
      }
    }
  );
