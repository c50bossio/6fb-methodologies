...
export const WHISPER_COSTS = {
  COST_PER_MINUTE: parseFloat(
    process.env.NEXT_PUBLIC_WHISPER_COST_PER_MINUTE || '0.006'
  ),
  MAX_DAILY_COST: parseFloat(
    process.env.NEXT_PUBLIC_MAX_DAILY_TRANSCRIPTION_COST || '50.00'
  ),
  WARNING_COST: parseFloat(
    process.env.NEXT_PUBLIC_WARN_TRANSCRIPTION_COST || '25.00'
  ),
};
...
export function validateAudioFile(file: File): AudioValidationResult {
  try {
    ...
    const estimatedDuration = Math.max(
      1,
      Math.floor((file.size / (1024 * 1024)) * 60)
    );
    ...
    const estimatedCost =
      (estimatedDuration / 60) * WHISPER_COSTS.COST_PER_MINUTE;
...
export async function transcribeAudio(
  ...
  try {
    ...
    formData.append(
      'model',
      process.env.NEXT_PUBLIC_WHISPER_MODEL || 'whisper-1'
    );
    ...
    if (options.temperature !== undefined) {
      formData.append(
        'temperature',
        Math.max(0, Math.min(1, options.temperature)).toString()
      );
    }
...
export function getUserUsageStats(
  ...
  for (const [action, actionData] of userLimits.entries()) {
    if (actionData.resetTime > now && stats[action as keyof typeof stats]) {
      ...
      statEntry.remaining = Math.max(
        0,
        statEntry.limit - actionData.count
      ) as typeof statEntry.remaining;
