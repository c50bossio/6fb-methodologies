...
export type NoteCategory = 'session-note' | 'reflection' | 'action-item' | 'question' | 'insight' | 'follow-up'
...
export function saveNote(userId: string, noteData: CreateNoteData): string {
  try {
    ...
    // Get existing notes
    const existingNotes = loadUserProgress<SessionNote[]>(userId, 'session_notes') || []
...
export function updateNote(userId: string, noteId: string, updates: Partial<CreateNoteData>): boolean {
  try {
    const notes = loadUserProgress<SessionNote[]>(userId, 'session_notes') || []
...
export function getUserNotes(userId: string, options: NoteQueryOptions = {}): SessionNote[] {
  try {
...
export function deleteNote(userId: string, noteId: string): boolean {
  try {
    const notes = loadUserProgress<SessionNote[]>(userId, 'session_notes') || []
    const filteredNotes = notes.filter(n => !(n.id === noteId && n.userId === userId))
...
export function getSessionNotes(userId: string, day: 1 | 2, session: string): SessionNote[] {
  return getUserNotes(userId, { day, session })
}

// Get notes by speaker
export function getSpeakerNotes(userId: string, speaker: NoteSpeaker): SessionNote[] {
  return getUserNotes(userId, { speaker })
}

// Get recent notes
export function getRecentNotes(userId: string, limit: number = 10): SessionNote[] {
  const notes = getUserNotes(userId)
...
export function exportNotes(userId: string, format: 'json' | 'markdown' | 'csv' = 'json'): string {
  try {
    const notes = getUserNotes(userId)
    ...
    switch (format) {
      ...
      case 'markdown':
        ...
        notes.forEach(note => {
          ...
          if (note.tags.length > 0) markdown += `**Tags:** ${note.tags.join(', ')}\n`
          markdown += '\n'
      ...
      case 'csv':
        let csv = 'ID,Title,Content,Category,Speaker,Day,Session,Tags,Date,Private\n'
        notes.forEach(note => {
          const row = [
            note.id,
...
// Bulk operations
export function bulkUpdateNotes(userId: string, noteIds: string[], updates: Partial<CreateNoteData>): number {
  let updatedCount = 0
