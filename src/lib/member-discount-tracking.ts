...
class DiscountUsageStore {
  ...
  getStats(): { totalCount: number; totalDiscountGiven: number } {
    ...
    return {
      ...
      totalDiscountGiven: records.reduce(
 ...
 */
export async function checkMemberDiscountUsage(
  email: string
): Promise<DiscountUsageCheck> {
  try {
 ...
 */
export async function getMemberDiscountUsageStats(
  options: {
    startDate?: string;
    endDate?: string;
    cityId?: string;
  ...
  try {
    ...
    const totalDiscountGiven = records.reduce(
      (sum, record) => sum + record.discountAmountCents,
      0
    );
