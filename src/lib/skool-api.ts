...
export class SkoolAPIClient {
  ...
  async fetchActiveMembers(
    page = 1,
    limit = 100
  ): Promise<SkoolAPIResponse<SkoolAPIMember[]>> {
    try {
      console.log(
        `🔍 Fetching Skool members (page ${page}, limit ${limit})...`
      );
  ...
  async syncAllActiveMembers(): Promise<{
    ...
    try {
      ...
      while (hasMore) {
        ...
        for (const apiMember of members) {
          try {
            if (
              apiMember.isActive &&
              apiMember.subscriptionStatus === 'active'
            ) {
            ...
            result.errors.push(
              `Failed to import ${apiMember.email}: ${(error as Error).message}`
            );
          }
        }
      ...
      console.log(
        `✅ Bulk sync completed: ${result.imported} imported, ${result.skipped} skipped`
      );
  ...
  private normalizeMember(rawMember: any): SkoolAPIMember {
    return {
      ...
      firstName:
        rawMember.firstName ||
        rawMember.first_name ||
        rawMember.name?.split(' ')[0] ||
        'Unknown',
      lastName:
        rawMember.lastName ||
        rawMember.last_name ||
        rawMember.name?.split(' ').slice(1).join(' ') ||
        'Member',
      username: rawMember.username || rawMember.handle,
      avatarUrl:
        rawMember.avatarUrl || rawMember.avatar_url || rawMember.profile_image,
      joinedAt:
        rawMember.joinedAt ||
        rawMember.joined_at ||
        rawMember.created_at ||
        new Date().toISOString(),
      isActive:
        (rawMember.isActive ??
          rawMember.is_active ??
          rawMember.status === 'active') ||
        true,
      subscriptionStatus:
        rawMember.subscriptionStatus ||
        rawMember.subscription_status ||
        'active',
      lastPaymentDate: rawMember.lastPaymentDate || rawMember.last_payment_date,
      membershipLevel:
        rawMember.membershipLevel || rawMember.membership_level || 'basic',
    };
  }
}
...
export async function syncExistingSkoolMembers(): Promise<{
  ...
  if (!client) {
    return {
      ...
      imported: 0,
      message:
        'Skool API not configured. Check SKOOL_API_KEY and SKOOL_GROUP_URL environment variables.',
    };
  }
