  ...
  private async processJob(job: TranscriptionJob) {
    ...
    await db.query(
      `UPDATE transcription_jobs
       ...
       WHERE id = $6`,
      ['processing', job.startedAt, this.processorId, job.attempts, new Date(), job.id]
    )
    ...
    try {
      ...
      if (job.attempts < job.maxAttempts) {
        console.log(`Scheduling retry for job ${job.id} (attempt ${job.attempts + 1}/${job.maxAttempts})`)
