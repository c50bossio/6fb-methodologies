...
class PaymentAnalytics {
  ...
  async getPaymentMetrics(days: number = 7): Promise<PaymentMetrics> {
    ...
    try {
      ...
      const metrics: PaymentMetrics = {
        ...
        conversionRate:
          totalSessions > 0
            ? ((paidSessions / totalSessions) * 100).toFixed(2)
            : '0',
        totalRevenue: successful.reduce((sum, p) => sum + p.amount, 0) / 100,
        avgTransactionValue:
          successful.length > 0
            ? successful.reduce((sum, p) => sum + p.amount, 0) /
              successful.length /
              100
            : 0,
        successRate:
        ...
        paymentMethods: successful.reduce(
          (acc, p) => {
        ...
        currencies: successful.reduce(
          (acc, p) => {
            acc[p.currency.toUpperCase()] =
              (acc[p.currency.toUpperCase()] || 0) + 1;
            return acc;
          },
          {} as Record<string, number>
        ),
        topFailureReasons: failed
          .reduce(
            (acc, p) => {
          ...
          .slice(0, 5),
        geographicBreakdown: successful.reduce(
          (acc, p) => {
        ...
        timeToComplete: {
          ...
          median:
            completionTimes.sort()[Math.floor(completionTimes.length / 2)] || 0,
          p95:
            completionTimes.sort()[Math.floor(completionTimes.length * 0.95)] ||
            0,
        },
      };
  ...
  async getRealTimeMetrics(): Promise<RealTimeMetrics> {
    ...
    try {
      ...
      const activeSessions = recentSessions.data.filter(
        s => s.payment_status === 'unpaid' && s.status === 'open'
      ).length;

      const successfulCharges = recentCharges.data.filter(
        c => c.status === 'succeeded'
      );
      const failedCharges = recentCharges.data.filter(
        c => c.status === 'failed'
      );
      ...
      const metrics: RealTimeMetrics = {
        activeSessions,
        currentRevenue:
          successfulCharges.reduce((sum, c) => sum + c.amount, 0) / 100,
    ...
    try {
      ...
      if (conversionRate < 70) {
        recommendations.push(
          'Consider simplifying the checkout process - conversion rate is below industry average'
        );
        conversionOpportunities.push(
          'Optimize form fields and reduce required information'
        );
      }
      ...
      if (failureRate > 5) {
        recommendations.push(
          'High payment failure rate detected - review payment methods and fraud settings'
        );
        conversionOpportunities.push(
          'Add more payment options like Apple Pay or Google Pay'
        );
      }
      ...
      if (abandonmentRate > 30) {
        recommendations.push(
          'High cart abandonment - consider exit-intent popups or email recovery'
        );
        conversionOpportunities.push(
          'Implement payment recovery emails and retargeting'
        );
      }

      // Payment method insights
      const totalPayments = Object.values(metrics.paymentMethods).reduce(
        (a, b) => a + b,
        0
      );
      ...
      if (cardPayments / totalPayments > 0.9) {
        recommendations.push(
          'Most payments are cards - consider adding mobile wallets for better UX'
        );
      }
