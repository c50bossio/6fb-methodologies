...
const generateMockSchedules = (): WorkshopSchedule[] => {
  ...
  for (let i = 7; i < 60; i += 7) {
    // Weekly workshops, starting next week
    ...
    if (getDeterministicCount(`${dateString}-morning-full`, 100) < 10) {
      // 10% chance of being full
      morningSlot.registered = morningSlot.capacity;
      morningSlot.isAvailable = false;
    }

    if (getDeterministicCount(`${dateString}-afternoon-unavailable`, 100) < 5) {
    ...
    const schedule: WorkshopSchedule = {
      ...
      description:
        'Master the proven systems that have helped barbershops scale to 6-figure revenues. Learn customer acquisition, retention strategies, pricing optimization, and operational excellence.',
      date: dateString,
      location: {
      ...
      schedules = schedules.filter(
        schedule =>
          ...
          schedule.timeSlots.some(
            slot => slot.isAvailable && slot.registered < slot.capacity
          )
      );
    }
