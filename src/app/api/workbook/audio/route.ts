...
async function validateTranscriptionLimits(userId: string, durationMinutes: number) {
  const user = await db.queryOne(
    'SELECT * FROM workbook_users WHERE id = $1',
    [userId]
  )
  ...
  // Check daily limit
  const availableMinutes = user.daily_transcription_limit_minutes - user.daily_transcription_used_minutes
  if (availableMinutes < durationMinutes) {
    throw new ValidationError(`Daily transcription limit exceeded. Available: ${availableMinutes} minutes`)
  }
  ...
  if ((user.monthly_transcription_cost_cents + estimatedCostCents) > user.monthly_cost_limit_cents) {
...
async function updateTranscriptionUsage(userId: string, durationMinutes: number, costCents: number) {
  await db.query(`
...
// POST /api/workbook/audio - Upload audio for transcription
export async function POST(request: NextRequest) {
  try {
    ...
    if (!hasPermission(auth.session, WORKBOOK_PERMISSIONS.TRANSCRIBE_AUDIO)) {
      return NextResponse.json({ error: 'Insufficient permissions for audio transcription' }, { status: 403 })
    }
    ...
    const chunkNumber = parseInt(formData.get('chunkNumber') as string || '0')
    ...
    const estimatedDurationMinutes = Math.ceil(audioFile.size / (1024 * 1024 * 0.5)) // Rough estimate

    // Validate transcription limits
    ...
    const audioRecord = await db.queryOne(`
      INSERT INTO audio_recordings (
        id, session_id, user_id, chunk_number, file_size_bytes,
        format, upload_status, metadata, created_at
    ...
    try {
      ...
      const actualDurationMinutes = Math.ceil((transcriptionResponse.duration || 0) / 60)
      ...
      await updateTranscriptionUsage(auth.session.userId, actualDurationMinutes, actualCostCents)
      ...
      return NextResponse.json({
        ...
        audio_recording: {
          id: recordingId,
          duration_seconds: transcriptionResponse.duration
        },
...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ error: 'Failed to fetch audio recordings' }, { status: 500 })
  }
}