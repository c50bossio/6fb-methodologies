...
import { extractToken, verifyToken, validateSession, WorkbookRole } from '@/lib/workbook-auth'
...
async function authenticateAdmin(request: NextRequest) {
  ...
  if (session!.role !== WorkbookRole.VIP && !session!.permissions.includes('admin_access')) {
  ...
  try {
    ...
    if (detailed) {
      ...
      response.system_health = {
        ...
        avg_processing_time_seconds: parseFloat(systemHealth?.avg_processing_time_seconds || '0'),
  ...
  try {
    ...
    switch (action) {
      case 'cleanup':
        ...
        const cleanedCount = await require('@/lib/transcription-queue').default.cleanupOldJobs(hours)
      ...
      case 'cancel_job':
        if (!jobId) {
          return NextResponse.json({ error: 'Job ID is required for cancel operation' }, { status: 400 })
        }

        const cancelled = await require('@/lib/transcription-queue').cancelTranscriptionJob(jobId)
