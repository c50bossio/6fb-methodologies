...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!hasPermission(auth.session, WORKBOOK_PERMISSIONS.SAVE_PROGRESS)) {
      return NextResponse.json({ error: 'Insufficient permissions to update progress' }, { status: 403 })
    }
    ...
    if (existingProgress) {
      ...
      if (timeSpentSeconds > 0) {
        updates.push(`time_spent_seconds = time_spent_seconds + $${paramIndex}`)
      ...
      params.push(auth.session.userId, moduleId)
  ...
  try {
    ...
    await db.transaction(async (client) => {
      for (const update of updates) {
        const { moduleId, progressPercentage, completed, timeSpentSeconds, metadata } = update
