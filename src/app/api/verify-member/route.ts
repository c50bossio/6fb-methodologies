...
const fallbackMemberData: Record<
  string,
  { name: string; membershipType: string; joinDate: string }
> = {
  'dre@6fb.com': {
    name: 'Dre',
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(
      `CSV verification result for: ${normalizedEmail} - not found in CSV members`
    );
    ...
    console.log(
      `Skool verification result for: ${normalizedEmail} - not found in Skool members`
    );
    ...
    try {
      ...
      if (stripeVerification.isVerified && stripeVerification.member) {
        console.log(
          `✅ Stripe verification successful for: ${normalizedEmail}`
        );
      ...
      console.log(
        `Stripe verification failed for: ${normalizedEmail}, reason: ${stripeVerification.error}`
      );
    } catch (stripeError) {
    ...
    console.log(
      `Member verification failed for: ${normalizedEmail} - not found in Stripe or fallback list`
    );
    ...
    if (email) {
      ...
      return NextResponse.json({
        ...
        fallback: {
          ...
          data: fallbackVerification
            ? fallbackMemberData[normalizedEmail]
            : null,
        },
        priority: csvVerification.isVerified
          ? 'csv'
          : skoolVerification.isVerified
            ? 'skool'
            : stripeVerification.isVerified
              ? 'stripe'
