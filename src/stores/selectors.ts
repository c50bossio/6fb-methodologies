...
export const useSubscriptionTier = () => useAuthStore((state) => state.user?.subscriptionTier);
...
export const useFilteredModules = () => useWorkbookStore((state) => state.getFilteredModules());
export const useCurrentModule = () => useWorkbookStore((state) => {
...
export const useFilteredNotes = () => useWorkbookStore((state) => state.getFilteredNotes());
export const useActiveNote = () => useWorkbookStore((state) => {
  const { notes, activeNoteId } = state;
  return notes.find(n => n.id === activeNoteId) || null;
});

export const useRecordings = () => useWorkbookStore((state) => state.recordings);
export const useTranscriptions = () => useWorkbookStore((state) => state.transcriptions);

export const useProgress = () => useWorkbookStore((state) => state.progress);
export const useOverallProgress = () => useWorkbookStore((state) => state.getOverallProgress());

export const useModuleProgress = (moduleId: string) =>
  useWorkbookStore((state) => state.getModuleProgress(moduleId));

export const useCurrentLessons = () => useWorkbookStore((state) => {
  const { lessons, currentModuleId } = state;
  return currentModuleId ? lessons[currentModuleId] || [] : [];
});

export const useCurrentLesson = () => useWorkbookStore((state) => {
...
export const useModulesByDifficulty = (difficulty: 'beginner' | 'intermediate' | 'advanced') =>
  useWorkbookStore((state) => {
    return state.modules.filter(m => m.difficulty === difficulty);
  });

export const useAvailableTags = () => useWorkbookStore((state) => state.getAvailableTags());

export const useSearchResults = () => useWorkbookStore((state) => {
...
export const useWorkbookStats = () =>
  useWorkbookStore((state) => {
    ...
    const completedModules = Object.values(state.progress).filter(p => p.status === 'completed').length;

    const totalTimeSpent = Object.values(state.progress).reduce((total, p) => total + p.timeSpentMinutes, 0);
    ...
    return {
      ...
      totalTimeSpent,
      averageTimePerModule: totalModules > 0 ? Math.round(totalTimeSpent / totalModules) : 0,
    };
  });
...
export const useCurrentSession = () => useLiveSessionStore((state) => state.currentSession);
export const useSessionParticipants = () => useLiveSessionStore((state) => state.participants);
export const useLocalParticipant = () => useLiveSessionStore((state) => state.getLocalParticipant());

export const useChatMessages = () => useLiveSessionStore((state) => state.chatMessages);
export const useUnreadMessageCount = () => useLiveSessionStore((state) => state.unreadMessageCount);

export const useActivePolls = () => useLiveSessionStore((state) => state.activePolls);
export const useCurrentPoll = () => useLiveSessionStore((state) => {
  const { activePolls, activePollId } = state;
  return activePolls.find(p => p.id === activePollId) || null;
});

export const useQAQuestions = () => useLiveSessionStore((state) => state.qaQuestions);

export const useMediaState = () => useLiveSessionStore((state) => state.mediaState);
export const useConnectionState = () => useLiveSessionStore((state) => state.connectionState);

export const useSessionPermissions = () => useLiveSessionStore((state) => state.permissions);
export const useIsSessionHost = () => useLiveSessionStore((state) => state.isHost);
export const useIsSessionModerator = () => useLiveSessionStore((state) => state.isModerator);

// Derived selectors
export const useActiveSpeakers = () => useLiveSessionStore((state) => state.getActiveSpeakers());

export const useSessionDuration = () => useLiveSessionStore((state) => state.getSessionDuration());
export const useFormattedSessionTime = () => useLiveSessionStore((state) => state.formatSessionTime());

export const useCanPerformSessionAction = (action: string) =>
  useLiveSessionStore((state) => state.canPerformAction(action));

export const useConnectionQuality = () => useLiveSessionStore((state) => {
...
export const useSessionStats = () => useLiveSessionStore((state) => {
  ...
  return {
    ...
    participantsWithVideo: participants.filter(p => p.mediaState.videoEnabled).length,
    participantsWithAudio: participants.filter(p => p.mediaState.audioEnabled && !p.mediaState.isMuted).length,
  };
});
...
export const useWorkbookNavigation = () => {
  ...
  const getNextLesson = () => {
    ...
    const currentModuleLessons = useWorkbookStore.getState().lessons[currentModule.id] || [];
    const currentIndex = currentModuleLessons.findIndex(l => l.id === currentLesson.id);
    ...
    if (currentModuleIndex < modules.length - 1) {
      ...
      const nextModuleLessons = useWorkbookStore.getState().lessons[nextModule.id] || [];
  ...
  const getPreviousLesson = () => {
    ...
    if (currentModuleIndex > 0) {
      ...
      const prevModuleLessons = useWorkbookStore.getState().lessons[prevModule.id] || [];
