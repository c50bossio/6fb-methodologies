...
import { Clock, Users, Target, TrendingUp, DollarSign, BarChart } from 'lucide-react'
        ...
        <div className="text-center mb-16">
          ...
          >
            ...
            <p className="body-lg max-w-3xl mx-auto text-text-secondary">
              Every minute is designed to deliver maximum value. You'll leave with
              actionable strategies, clear KPIs, and a personalized 90-day plan.
            </p>
          </motion.div>
        </div>
              ...
              {day1Schedule.map((item, index) => {
                ...
                return (
                  <motion.div
                    ...
                    viewport={{ once: true }}
                  >
                    <Card className={`${item.type === 'session' ? 'border-tomb45-green/20' : ''}`}>
                        ...
                        <div className="flex items-start gap-3">
                          ...
                          <div className="flex-1">
                            <div className="flex items-center gap-2 mb-1">
                              ...
                              {item.type === 'vip' && (
                                <Badge variant="success" className="text-xs">VIP Only</Badge>
                              )}
                            </div>
              ...
              {day2Schedule.map((item, index) => {
                ...
                return (
                  <motion.div
                    ...
                    viewport={{ once: true }}
                  >
                    <Card className={`${item.type === 'session' ? 'border-tomb45-green/20' : ''} ${item.type === 'vip' ? 'border-purple-500/20' : ''}`}>
                      <CardContent className="p-4">
                        <div className="flex items-start gap-3">
                          <div className={`${item.type === 'vip' ? 'bg-purple-500/10' : 'bg-tomb45-green/10'} p-2 rounded-lg flex-shrink-0`}>
                            <Icon className={`w-4 h-4 ${item.type === 'vip' ? 'text-purple-500' : 'text-tomb45-green'}`} />
                          </div>
                          <div className="flex-1">
                            <div className="flex items-center gap-2 mb-1">
                              <span className={`text-sm font-mono ${item.type === 'vip' ? 'text-purple-500' : 'text-tomb45-green'}`}>
                                {item.time}
                              </span>
                              {item.type === 'vip' && (
                                <Badge className="text-xs bg-purple-500 text-white">VIP Only</Badge>
                              )}
                            </div>
                            <h4 className="font-semibold text-text-primary mb-1">
                              {item.title}
                            ...
                            {item.details && (
                              <ul className="text-xs text-text-muted space-y-1">
                                {item.details.map((detail, i) => (
                                  <li key={i} className="flex items-start gap-1">
                                    <span className={`w-1 h-1 ${item.type === 'vip' ? 'bg-purple-500' : 'bg-tomb45-green'} rounded-full mt-2 flex-shrink-0`} />
                                    {detail}
                                  </li>
                                ))}
                              </ul>
