...
export function WorkbookLogin({ onSuccess }: WorkbookLoginProps) {
  // Try to get auth context, but fallback gracefully if it fails
  let authContext;
  try {
    authContext = useWorkbookAuth();
  } catch (error) {
    console.error('Auth context failed, using fallback:', error);
    authContext = {
  ...
  const [forceEnabled, setForceEnabled] = useState(true); // Default to enabled

  // Emergency fallback: ensure inputs are always enabled after component mounts
  useEffect(() => {
    const timer = setTimeout(() => {
      console.log('🚨 Emergency fallback: Ensuring inputs are enabled');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setError(
          result.error ||
            'Invalid email or access code. Please check your credentials.'
        );
      }
    } catch (err) {
    ...
    <div className='min-h-screen bg-background-primary flex items-center justify-center p-4'>
      <div className='w-full max-w-md space-y-6'>
            ...
            <form onSubmit={handleSubmit} className='space-y-4'>
              <div>
                <Input
                  ...
                  disabled={forceEnabled ? false : isSubmitting || isLoading}
                  autocomplete='email'
                  className='w-full'
                  required
                />
              ...
              <div>
                <Input
                  ...
                  helperText='Enter the access code from your registration email'
                  required
                />
              </div>
              ...
              <Button
                ...
                className='w-full'
                onClick={() => {
                  console.log('🔘 Login button clicked with state:', {
                    email,
        ...
        <div className='text-center text-sm text-text-secondary'>
          <p>Your access level is determined by your 6FB membership status.</p>
          <p className='mt-1'>
            Need help? Contact{' '}
            <a
