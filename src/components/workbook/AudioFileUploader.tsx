...
const AudioPreview: React.FC<{
  ...
  const progress =
    fileInfo.duration > 0 ? (currentTime / fileInfo.duration) * 100 : 0;
...
export const AudioFileUploader: React.FC<AudioFileUploaderProps> = ({
  ...
  return (
    <Card
      className={`bg-background-secondary border-border-primary shadow-dark-lg ${className}`}
    >
      <CardHeader>
      ...
      <CardContent className='space-y-4'>
        ...
        {fileInfo && uploadStatus === 'complete' && (
          <AudioPreview fileInfo={fileInfo} onRemove={handleRemoveFile} />
        )}
        ...
        <div className='text-xs text-text-muted space-y-1'>
          ...
          {sessionInfo && (
            <p>• Uploads will be tagged with current session info</p>
          )}
        </div>
