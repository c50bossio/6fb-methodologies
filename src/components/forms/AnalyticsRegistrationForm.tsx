...
const FORM_STEPS: FormStep[] = [
  {
    id: 1,
    title: 'Quick Details',
    description: 'Just the essentials to secure your spot',
  },
...
export function AnalyticsRegistrationForm({
  searchParams,
}: AnalyticsRegistrationFormProps) {
  ...
  // Analytics hooks
  const { trackStepProgress, trackStepValidation, trackFieldInteraction } =
    useFormTracking('workshop_registration');
  const {
  ...
  useEffect(() => {
    ...
    const originalPrice = parseFloat(
      searchParams.get('originalPrice') || '1000'
    );
  ...
  const handleNext = () => {
    if (validateStep(currentStep) && currentStep < FORM_STEPS.length) {
      ...
      trackCustomTiming(
        `step_${currentStep}_completion_time`,
  ...
  const handlePrevious = () => {
    if (currentStep > 1) {
      ...
      trackStepProgress(
        currentStep - 1,
        FORM_STEPS[currentStep - 2].title,
        formData
      );
    }
  };
    ...
    <div className='min-h-screen bg-background-primary py-12'>
      <div className='container-custom max-w-2xl'>
        ...
        >
          ...
          <h1 className='heading-lg mb-4'>You're Almost In! 🎆</h1>
          <p className='body-md text-text-secondary'>
            Just 2 quick steps to secure your spot at the 6FB Methodologies
            Workshop
          </p>
        ...
        <motion.div
          ...
          onViewportEnter={() =>
            trackSectionView(`registration_step_${currentStep}`)
          }
        >
          <Card className='mb-8'>
