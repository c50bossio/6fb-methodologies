...
function SecurityBanner() {
  const { session, canRecordAudio, canTranscribeAudio, canAccessVIPContent } = useWorkbookAuth()

  const getSecurityLevel = () => {
    if (canAccessVIPContent) return { level: 'VIP', color: 'text-tomb45-green', bg: 'bg-green-50' }
    if (canTranscribeAudio) return { level: 'Premium', color: 'text-blue-600', bg: 'bg-blue-50' }
    ...
    <div className={`${security.bg} border-l-4 border-l-tomb45-green p-4 mb-6`}>
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h3 className="font-medium text-text-primary">Secure Session Active</h3>
            <p className="text-sm text-text-secondary">
              Authenticated as <span className={`font-medium ${security.color}`}>{session?.name}</span> with <span className={`font-medium ${security.color} uppercase`}>{security.level}</span> access
            </p>
          </div>
        </div>
        <div className="flex items-center space-x-4">
          <div className="text-right text-sm">
            ...
            <p className="font-medium text-text-primary">
              {session?.exp ? new Date(session.exp * 1000).toLocaleTimeString() : 'Unknown'}
            </p>
          </div>
        </div>
      </div>
          ...
          <div>
            ...
            <div className="space-y-1 text-sm">
              ...
              <div>
                ...
                <span className="ml-2 text-tomb45-green font-medium uppercase">{session?.role}</span>
              </div>
              <div>
                <span className="text-text-secondary">User ID:</span>
                <span className="ml-2 text-text-primary font-mono text-xs">{session?.userId}</span>
              </div>
            </div>
          </div>
          <div>
...
function SecurityWarning({ feature, requiredLevel }: { feature: string; requiredLevel: string }) {
  return (
        ...
        <div className="flex items-center space-x-3">
          ...
          <div>
            ...
            <p className="text-sm text-yellow-700">
              {feature} requires {requiredLevel} access. Your current role may not have sufficient permissions.
            </p>
          </div>
        </div>
      </CardContent>
...
function PermissionGuard({
  ...
  if (permission && !hasPermission(permission)) {
    return <SecurityWarning feature={feature || 'This feature'} requiredLevel={requiredLevel} />
  }
...
export function SecureWorkbookWrapper({ children }: SecureWorkbookWrapperProps) {
