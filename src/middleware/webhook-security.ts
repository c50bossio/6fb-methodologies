...
class WebhookSecurity {
  ...
  static verifyWebhook(
    ...
    return this.verifyGenericSignature(payload, signature, config.secret, config.algorithm)
  }
...
export function withWebhookSecurity(config: WebhookConfig) {
  return function (handler: (req: NextRequest, payload: string) => Promise<Response>) {
...
export async function verifyWebhookWithReplayProtection(
  ...
  const signature = req.headers.get(config.headerName) || ''
  const timestamp = config.validateTimestamp ? Math.floor(Date.now() / 1000) : undefined
...
export function validateWebhookContentType(req: NextRequest, expectedType: string = 'application/json'): boolean {
