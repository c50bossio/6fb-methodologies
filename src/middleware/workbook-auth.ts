...
import { verifyToken, extractToken, validateSession, hasPermission, WORKBOOK_SECURITY_HEADERS } from '@/lib/workbook-auth'
...
export async function workbookAuthMiddleware(
  ...
  try {
    ...
    if (process.env.NODE_ENV === 'development') {
      ...
      response.headers.set('X-Workbook-Permissions', session.permissions.join(','))
    }
...
export async function protectedWorkbookMiddleware(
  ...
  const authResult = await workbookAuthMiddleware(request, options.requiredPermission)
  if (authResult) {
    return authResult
  }
  ...
  if (options.rateLimitAction) {
    const rateLimitResult = await workbookRateLimitMiddleware(request, options.rateLimitAction)
    if (rateLimitResult) {
...
export function isWorkbookPath(pathname: string): boolean {
  return pathname.startsWith('/workbook') || pathname.startsWith('/api/workbook')
}
