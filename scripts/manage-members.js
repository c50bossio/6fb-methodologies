...
if (
  process.env.SKOOL_STRIPE_SECRET_KEY &&
  !process.env.SKOOL_STRIPE_SECRET_KEY.includes('REPLACE')
) {
  ...
  console.log(
    colorize('yellow', '⚠️  Skool Stripe Express account not configured')
  );
}
...
async function verify6FBMembership(email) {
  try {
    ...
    console.log(
      colorize('blue', `🔍 Searching for customer: ${normalizedEmail}`)
    );
    ...
    console.error(
      colorize('red', '❌ Error verifying membership:'),
      error.message
    );
    return {
      isVerified: false,
...
async function checkCustomerMembership(customer) {
  try {
    ...
    console.log(
      colorize('cyan', `  💰 Successful payments: ${successfulPayments.length}`)
    );
    console.log(
      colorize('cyan', `  🧾 Paid invoices: ${invoices.data.length}`)
    );
    ...
    console.error(
      colorize('red', '❌ Error checking customer membership:'),
      error.message
    );
    return {
      isVerified: false,
...
async function verifyCommand(email) {
  ...
  if (!email) {
    ...
    console.log(
      colorize('yellow', 'Usage: node scripts/manage-members.js verify <email>')
    );
    process.exit(1);
  }
  ...
  if (result.isVerified && result.member) {
    ...
    console.log(
      colorize('cyan', `🔴 Active: ${result.member.isActive ? 'Yes' : 'No'}`)
    );
    console.log(
      colorize('cyan', `📅 Joined: ${result.member.joinDate.split('T')[0]}`)
    );
    if (result.member.lastPayment) {
      console.log(
        colorize(
          'cyan',
          `💰 Last Payment: ${result.member.lastPayment.split('T')[0]}`
        )
      );
    }
    console.log(
      colorize('cyan', `🆔 Customer ID: ${result.member.customerId}`)
    );
  } else {
  ...
  if (result.isVerified) {
    console.log(
      colorize(
        'green',
        '✅ Eligible for 6FB member discount (20% off GA tickets)'
      )
    );
  } else {
...
async function syncCommand(options = {}) {
  ...
  try {
        ...
        if (customer.email) {
          ...
          if (membershipInfo.isVerified) {
            ...
            console.log(
              colorize(
                'green',
                `✅ ${customer.email} - ${membershipInfo.member.membershipType}`
              )
            );
          } else {
            console.log(
              colorize(
                'yellow',
                `⚠️  ${customer.email} - No qualifying payments`
              )
            );
          }
        } else {
          console.log(
            colorize('red', `❌ Customer ${customer.id} - No email address`)
          );
        }
      }
    ...
    console.log('-' * 30);
    console.log(
      colorize('cyan', `👥 Total customers checked: ${customerCount}`)
    );
    console.log(
      colorize('green', `✅ Verified members found: ${verifiedCount}`)
    );
    console.log(
      colorize(
        'blue',
        `📈 Verification rate: ${((verifiedCount / customerCount) * 100).toFixed(1)}%`
      )
    );
    ...
    if (membersList.length > 0) {
      ...
      membersList.forEach((member, index) => {
        console.log(
          colorize(
            'green',
            `${index + 1}. ${member.email} (${member.membershipType})`
          )
        );
      });
    }
...
async function listCommand(options = {}) {
  ...
  console.log(
    colorize('bright', `🗂️  Recent Stripe Customers (Last ${limit})`)
  );
  console.log('=' * 50);
  ...
  try {
    ...
    console.error(
      colorize('red', '❌ Failed to list customers:'),
      error.message
    );
  }
}
...
async function statsCommand() {
  ...
  try {
    ...
    console.log(
      colorize(
        'green',
        `💰 Recent successful payments: ${successfulPayments.length}`
      )
    );
    console.log(
      colorize('blue', `💵 Recent revenue: $${(totalRevenue / 100).toFixed(2)}`)
    );
    console.log(
      colorize(
        'magenta',
...
async function testCommand() {
  ...
  for (const email of testEmails) {
    ...
    if (result.isVerified) {
      console.log(
        colorize('green', `✅ Verified - ${result.member.membershipType}`)
      );
    } else {
...
function showHelp() {
  ...
  console.log(
    colorize(
      'white',
      '  node scripts/manage-members.js verify user@example.com'
    )
  );
  console.log(colorize('white', '  node scripts/manage-members.js sync'));
  console.log(
    colorize('white', '  node scripts/manage-members.js list --limit=20')
  );
  console.log('');
}
