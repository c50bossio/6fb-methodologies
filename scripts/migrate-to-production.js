    ...
    const replacements = [
      {
        ...
        replace:
          '// Workbook User Management (Database-powered for production)',
      },
      {
...
import {
  ...
  checkDatabaseHealth
      ...
      {
        search:
          /\/\/ In-memory storage for workbook users \(for development\)[\s\S]*?const workbookUsers = new Map<string, WorkbookUser>\(\)/,
        replace: `// Database storage for workbook users (production)
// Using PostgreSQL for persistent storage`,
      },
      {
        search:
          /export function storeWorkbookUser\(user: WorkbookUser\): void \{[\s\S]*?\}/,
  ...
  return await dbStoreWorkbookUser(user)
      ...
      {
        search:
          /export async function verifyWorkbookPassword\(email: string, password: string\): Promise<boolean> \{[\s\S]*?\}/,
  ...
  return await dbVerifyWorkbookPassword(email, password)
      ...
      {
        search:
          /export function getWorkbookUser\(email: string\): WorkbookUser \| null \{[\s\S]*?\}/,
