...
async function getAccountInfo() {
  try {
    ...
    if (account.business_profile) {
      console.log(`Business Name: ${account.business_profile.name || 'Not specified'}`)
      console.log(`Business URL: ${account.business_profile.url || 'Not specified'}`)
    }

    if (account.settings) {
      console.log(`Dashboard: ${account.settings.dashboard ? account.settings.dashboard.display_name : 'Not available'}`)
    }
    ...
    console.log(`Total customers accessible: ${customers.data.length}+ (showing first 5)`)
    ...
    paymentIntents.data.forEach((payment, index) => {
      ...
      const created = new Date(payment.created * 1000).toISOString().split('T')[0]
      console.log(`${index + 1}. $${amount} ${payment.currency.toUpperCase()} - ${status} (${created})`)
    })
    ...
    for (const customer of customers.data) {
      if (customer.email && skoolKeywords.some(keyword =>
        customer.email.toLowerCase().includes(keyword) ||
        (customer.description && customer.description.toLowerCase().includes(keyword))
      )) {
        skoolRelated = true
        break
      }
    ...
    for (const payment of paymentIntents.data) {
      if (payment.description && skoolKeywords.some(keyword =>
    ...
    if (skoolRelated) {
      console.log('✅ This appears to be connected to a Skool/6FB related Stripe account')
    } else {
