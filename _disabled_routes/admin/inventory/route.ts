  ...
  try {
    ...
    const responseData: any = {
      summary: {
        ...
        lowInventoryCities: inventoryStatuses.filter(s =>
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!operation || !data || !authorizedBy) {
      return NextResponse.json(
        { success: false, error: 'Missing required fields: operation, data, authorizedBy' },
        { status: 400 }
      )
    }
    ...
    switch (operation) {
      ...
      default:
        return NextResponse.json(
          { success: false, error: 'Invalid operation. Must be expand, reset, or alert' },
          { status: 400 }
        )
    }
...
async function generateInventoryAnalytics(statuses: any[]) {
  const analytics = {
    ...
    cityBreakdown: statuses.map(s => ({
      ...
      fillRate: {
        ...
        vip: s.actualLimits.vip > 0 ? (s.sold.vip / s.actualLimits.vip) * 100 : 0
      },
    ...
    alerts: {
      criticalCities: statuses.filter(s =>
        s.publicAvailable.ga <= 2 || s.publicAvailable.vip <= 1
      ).map(s => s.cityId),
      lowInventoryCities: statuses.filter(s =>
        s.publicAvailable.ga <= 5 || s.publicAvailable.vip <= 3
      ).map(s => s.cityId),
  ...
  if (analytics.totalPublicCapacity.ga > 0) {
    analytics.sellThroughRate.ga = (analytics.totalSold.ga / analytics.totalPublicCapacity.ga) * 100
  }
  if (analytics.totalPublicCapacity.vip > 0) {
    analytics.sellThroughRate.vip = (analytics.totalSold.vip / analytics.totalPublicCapacity.vip) * 100
  }
...
async function handleBulkReset(data: any, authorizedBy: string) {
  ...
  console.log('bulk_inventory_reset', {
    cities,
    authorizedBy,
    reason,
    successCount,
...
async function handleManualAlert(data: any, authorizedBy: string) {
  ...
  console.log('manual_admin_alert', {
    message,
    urgency: urgency || 'medium',
    authorizedBy,
    timestamp: new Date().toISOString()
  }, 'inventory', urgency || 'medium')
