...
async function addAdminStatistics(
  ...
  try {
    ...
    const totalSize = userUtilizationResult.rows.reduce(
      (sum, row) => sum + parseInt(row.total_size, 10),
      0
    );
    ...
    data!.storage.utilizationByUser = userUtilizationResult.rows.map(row => ({
      ...
      percentage:
        totalSize > 0 ? (parseInt(row.total_size, 10) / totalSize) * 100 : 0,
    }));
    ...
    moduleBreakdownResult.rows.forEach(row => {
      data!.breakdown.filesByModule[row.module_name] = parseInt(row.file_count, 10);
      data!.breakdown.sizeByModule[row.module_name] = parseInt(row.total_size, 10);
    });
    ...
    data!.activity.downloadsByDay = downloadActivityResult.rows.map(row => ({
      ...
      count: parseInt(row.download_count, 10),
    }));
    ...
    data!.storage.largestFiles = largestFilesResult.rows.map(row => ({
      id: row.id,
      fileName: row.file_name,
      size: parseInt(row.file_size_bytes, 10),
      sizePretty: formatBytes(parseInt(row.file_size_bytes, 10)),
