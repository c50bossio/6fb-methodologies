...
export async function GET(
  ...
  try {
    ...
    const rawExpiresIn = parseInt(searchParams.get('expiresIn') || '3600', 10); // 1 hour default
    const inline = searchParams.get('inline') === 'true';
    const download = searchParams.get('download') === 'true';

    // Validate expiration time (max 24 hours, min 60 seconds)
    const maxExpirationTime = 24 * 60 * 60; // 24 hours
    const expiresIn = isNaN(rawExpiresIn) || rawExpiresIn < 60 ? 3600 : rawExpiresIn;
