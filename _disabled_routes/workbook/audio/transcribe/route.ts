...
async function validateTranscriptionLimits(
  ...
  try {
    ...
    if (!user) {
      console.warn('⚠️ User not found in database, using default limits for development');
      // Return mock user limits for development
      return {
        id: userId,
    ...
    console.warn('⚠️ Database error during user validation, using default limits:', error);
    // Return mock user limits if database is unavailable
    return {
      id: userId,
      daily_transcription_limit_minutes: 120,
      daily_transcription_used_minutes: 0,
...
export async function POST(request: NextRequest) {
  try {
    ...
    const rawChunkSizeMinutes = parseInt(
      (formData.get('chunkSizeMinutes') as string) || '5',
      10
    );
    const chunkSizeMinutes = isNaN(rawChunkSizeMinutes) || rawChunkSizeMinutes < 1 ? 5 : Math.min(rawChunkSizeMinutes, 30);
