...
export async function POST(request: NextRequest) {
  try {
    ...
    const dbUserId = getDbUserId(auth.session.userId);
    ...
    try {
      ...
      console.warn('❌ Database insert failed, using mock record:', dbError);

      // Create a mock record for the response if database fails
      audioRecord = {
        id: recordingId,
        ...
        s3_bucket: s3FileMetadata.bucket,
        title,
        description,
        tags: JSON.stringify(tags),
        waveform_data: JSON.stringify(s3FileMetadata.waveform || []),
        peaks_data: JSON.stringify(s3FileMetadata.peaks || []),
        duration_seconds: s3FileMetadata.duration || 0,
        is_public: isPublic,
