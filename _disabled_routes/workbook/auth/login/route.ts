...
export async function POST(request: NextRequest) {
  try {
    ...
    // Check if IP is locked out due to failed attempts with error handling
    try {
      if (isIPLockedOut && isIPLockedOut(clientIP)) {
      ...
      console.warn('IP lockout check failed:', error.message);
      // Continue with login - fail open for security functions
    }
    ...
    if (authResult.token) {
      ...
      const cookieOptions = {
        ...
        sameSite: 'lax' as const, // Lax for same-site requests
        maxAge: 24 * 60 * 60, // 24 hours
        path: '/', // Root path for broad access
        // Remove explicit domain for localhost compatibility
      };
