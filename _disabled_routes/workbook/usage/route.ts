...
export async function GET(request: NextRequest) {
  try {
    ...
    const analytics: UsageAnalytics = {
      ...
      activity_summary: {
        total_sessions: parseInt(sessionSummary.total_sessions, 10) || 0,
        total_session_time_minutes: Math.round(
          parseFloat(sessionSummary.total_session_time_minutes) || 0
        ),
        total_audio_recordings:
          parseInt(audioStats.total_audio_recordings, 10) || 0,
        total_transcriptions: parseInt(audioStats.total_transcriptions, 10) || 0,
        total_notes: parseInt(notesStats.total_notes, 10) || 0,
        last_activity:
      ...
      learning_progress: {
        modules_started: parseInt(learningProgress.modules_started, 10) || 0,
        modules_completed: parseInt(learningProgress.modules_completed, 10) || 0,
        lessons_completed: parseInt(learningProgress.lessons_completed, 10) || 0,
        overall_progress_percent: Math.round(
          parseFloat(learningProgress.overall_progress_percent) || 0
        ),
        current_streak_days: currentStreak,
        total_study_time_minutes:
          parseInt(learningProgress.total_study_time_minutes, 10) || 0,
      },
      ...
      engagement_metrics: {
        sessions_this_week: parseInt(engagementStats.sessions_this_week, 10) || 0,
        sessions_this_month: parseInt(engagementStats.sessions_this_month, 10) || 0,
        average_session_duration_minutes: Math.round(
          parseFloat(engagementStats.average_session_duration_minutes) || 0
        ),
        most_active_hour: parseInt(activeHourData?.hour, 10) || 9,
      ...
      content_interaction: {
        notes_created: parseInt(notesStats.total_notes, 10) || 0,
        action_items_created: parseInt(notesStats.action_items_created, 10) || 0,
        action_items_completed:
          parseInt(notesStats.action_items_completed, 10) || 0,
